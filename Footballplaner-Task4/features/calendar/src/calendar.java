...
public class calendar implements interfaces.calendars{
...
private static final int[] daysPerMonth = { 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 };
	...
	public interfaces.calendars createCalendar(int year, int month, interfaces.planer gp) {
		return new calendar(year, month, gp);
	}
	 ...
	 * @param gp Gameplaner
	 */
	public calendar(int year, int month, interfaces.planer gp) {
		...
		ArrayList<interfaces.IGame> dates=gp.getGames().get(0);
		for (int i = 0; i<dates.size(); i++) {
			if(dates.get(i).getFirstValue()==year&&dates.get(i).getSecondValue()==month) {
				calenderMonth[dates.get(i).getThirdValue()].addDate(i, dates.get(i).getFourthValue(), dates.get(i).getFifthValue());
			}
		}
	}
